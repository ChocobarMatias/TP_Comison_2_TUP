...
export const getOneDonador = async (req, res) => {
  ...
  try {
    const [result] = await pool.query(
      'SELECT * FROM donadores WHERE deleted_at IS NULL AND id = ?',
      [id]
    );
    if (result.length === 0)
...
export const postDonadores = async (req, res) => {
  ...
  try {
    ...
    if (result.affectedRows === 0)
      return res.json({ status: 404, message: 'No se pudo crear el donador' });
...
export const putDonadores = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'UPDATE donadores SET nombre = ?, apellido = ?, contacto = ? WHERE id = ?;',
      [nombre, apellido, contacto, id]
    );
    if (result.affectedRows === 0)
      return res.json({ status: 404, message: 'No se pudo actualizar el donador' });
    res.json({ status: 200, payload: result });
  } catch (error) {
    console.log(error);
