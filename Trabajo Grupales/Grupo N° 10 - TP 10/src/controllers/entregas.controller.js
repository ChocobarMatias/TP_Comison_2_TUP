...
export const getOneEntrega = async (req, res) => {
  ...
  try {
    const [result] = await pool.query(
      'SELECT * FROM entregas WHERE deleted_at IS NULL AND id = ?',
      [id]
    );
    if (result.length === 0)
...
export const postEntrega = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `INSERT INTO entregas (id_donador, id_producto, id_comedor, cantidad, observaciones) VALUES (?, ?, ?, ?, ?);`,
      [id_donador, id_producto, id_comedor, cantidad, observaciones]
    );
    if (result.affectedRows === 0)
      return res.json({ status: 404, message: 'No se pudo crear la entrega' });
...
export const putEntrega = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'UPDATE entregas SET id_donador = ?, id_producto = ?, id_comedor = ?, cantidad = ?, observaciones = ? WHERE id = ?;',
      [id_donador, id_producto, id_comedor, cantidad, observaciones, id]
    );
    if (result.affectedRows === 0)
      return res.json({ status: 404, message: 'No se pudo actualizar la entrega' });
    res.json({ status: 200, payload: result });
  } catch (error) {
    console.log(error);
